...
function getCurrencyCombinations(chainId, currencyA, currencyB) {
    const [tokenA, tokenB] = chainId
        ...
        : [undefined, undefined];
    const common = index_js_1.BASES_TO_CHECK_TRADES_AGAINST?.[chainId] ?? [];
    const additionalA = tokenA
        ? index_js_1.ADDITIONAL_BASES[chainId]?.[tokenA.address] ?? []
        : [];
    const additionalB = tokenB
        ? index_js_1.ADDITIONAL_BASES[chainId]?.[tokenB.address] ?? []
        : [];
    ...
    const combinations0 = [
        ...
        if (!chainId)
            return true;
        const customBases = index_js_1.CUSTOM_BASES[chainId];
...
//# sourceMappingURL=get-currency-combinations.js.map