...
class TinesToRouteProcessor {
    ...
    getRouteProcessorCode(route, toAddress) {
        ...
        if (route.status === index_js_2.RouteStatus.NoWay || route.legs.length === 0)
            return '';
    ...
    codeDistributeInitial(route) {
        ...
        const legsAddr = legs.map((l) => {
            ...
            return [
                l,
                startPoint === PoolCode_js_1.PoolCode.RouteProcessorAddress
        ...
        if (getTokenType(fromToken) === TokenType.BENTO)
            ...
            if (this.chainId === index_js_1.ChainId.CELO) {
        ...
        legsAddr.forEach(([leg, poolAddress]) => {
            const amount = leg !== lastLeg
                ? (0, index_js_2.getBigInt)(route.amountIn * leg.absolutePortion)
                : (0, index_js_2.getBigInt)(route.amountIn) - inputAmountPrevious;
    ...
    codeDistributeTokenShares(token, route) {
        ...
        const RPStartPointsNum = legs.filter((l) => {
            const pc = this.getPoolCode(l);
            const startPoint = pc.getStartPoint(l, route);
            return startPoint === PoolCode_js_1.PoolCode.RouteProcessorAddress;
        }).length;
        ...
        legs.forEach((l) => {
            const pc = this.getPoolCode(l);
            const startPoint = pc.getStartPoint(l, route);
            if (startPoint === PoolCode_js_1.PoolCode.RouteProcessorAddress) {
...
//# sourceMappingURL=tines-to-route-processor.js.map